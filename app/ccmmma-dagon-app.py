...
if __name__ == '__main__':
  ...
  data_dir=data_dir_base+start_date.strftime("%Y%m%d")+"/"+start_date.strftime("%Y%m%d")+"Z"+start_date.strftime("%H")
  ...
  taskMakeWpsNameList=batch.Batch("makeWpsNameList",command_dir_base+"/makeWpsNamelist."+wrf_model+" "+initial+" "+final)


  # The geogrid task executed using Slurm
  taskGeogrid=batch.Batch("geogrid","sbatch --wait "+command_dir_base+"/geogrid "+i_date+" workflow://makeWpsNameList/namelist.wps")

  # The ungrib task executed using Slurm
  taskUngrib=batch.Batch("ungrib","sbatch --wait "+command_dir_base+"/ungrib "+i_date+" "+data_dir+ " workflow://makeWpsNameList/namelist.wps")

  # the metgrid task executed using Slurm
  taskMetgrid=batch.Batch("metgrid","sbatch --wait "+command_dir_base+"/metgrid "+i_date+" workflow://makeWpsNameList/namelist.wps workflow://ungrib/FILE\* workflow://geogrid/geo_em.\*")
...
#    restartFile=restart_dir_base+"wrfrst_d\?\?_"+start_date.strftime("%Y-%m-%d")+"_00:00:00"
  ...
  taskMakeInputNameList=batch.Batch("makeInputNameList",command_dir_base+"/makeInputNamelist."+wrf_model+" "+i_date+" "+f_date+" 3 "+useRestart+" 1440")

  # The real task executed using Slurm
  taskReal=batch.Batch("real","sbatch --wait "+command_dir_base+"/real "+i_date+" workflow://makeInputNameList/namelist.input workflow://metgrid/met_em.\*")
  ...
  for day in range(0,days):
    ...
    taskMakeInputNameList=batch.Batch("makeInputNameList_"+str(day),command_dir_base+"/makeInputNamelist."+wrf_model+" "+i_date1s+" "+f_date1s+" 3 "+useRestart+" 1440")
    taskWrf=batch.Batch("wrf_"+str(day),"sbatch --wait "+command_dir_base+"/wrf "+i_date+" workflow://makeInputNameList_"+str(day)+"/namelist.input workflow://real/wrfbdy\* workflow://real/wrfinput\* "+restartFile)
    taskSaveRestart=batch.Batch("saveRestart_"+str(day),"cp -r workflow://wrf_"+str(day)+"/wrfrst_d??_"+f_date1.strftime("%Y-%m-%d")+"_00:00:00 "+restart_dir_base)

    taskPublish=batch.Batch("publishWrfOutput_"+str(day),"sbatch --wait "+command_dir_base+"/publishWrfOutput "+i_date+" "+i_date1s+" "+wrf_model+" workflow://wrf_"+str(day)+"/wrfout_")
